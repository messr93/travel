...
class OfferController extends Controller
{
    ...
    {
        $this->middleware(['auth', 'can:createOffer,App\Models\Offer']);            // must authenticated && not customer role
    }

    ///////////////////////////////////////////// Begin indexing ////////////////////////////////////////////////
    public function index(){
        return view('frontend.offers.index', ['pageTitle' => 'My offers']);
    }

    public function allData(){
        $user_id = auth()->user()->id;

        $allOffers = Offer::where('user_id', $user_id)->select('id', 'program_id', 'user_id', 'status')
        ...
        return Datatables::of($allOffers)
            ->addColumn('actions', 'frontend.offers.actions')
    ...
    public function create(){
        ...
        return view('frontend.offers.create', ['pageTitle' => 'Create Offer','programs' => $programs, 'activeLangs' => $activeLangs, 'cities' => $cities]);
    }

    public function insert(Request $request){
        $langs = explode(',', $request->input('offer_langs'));
    ...
    ///////////////////////////////////////////// Begin Editing ////////////////////////////////////////////////
    public function edit($oldLang, Offer $offer){

        $city_lang = (currentLang() == "ar")? "ar": "en";           // cause city only available in (en && ar)
        $offer = $offer->with(['user', 'offerContents', 'photos', 'prices'])->first();
        if(!isset($offer))
        ...
        return view('frontend.offers.edit', ['pageTitle' => 'Edit Offer', 'offer' => $offer,
    ...
    public function update(Request $request, Offer $offer){
        ...
        foreach ($offerOldLangs as $offerLang){
            if(!in_array($offerLang, $offerNewLangs)){                                           // remove offer content if un selected
                $offerContentDelete = OfferContent::where('offer_id', $offer->id)->where('lang_id', $offerLang)->first();
            ...
            $offerContentUpdate = OfferContent::where('offer_id', $offer->id)->where('lang_id', $offerLang)->first();                   // updated the offer content
    ...
    public function delete(Request $request){
        ...
        $this->authorize('updateOffer', $offer);                        // make sure user is authorized (updateOffer policy)
    ...
    protected function getCreateRules($request, $langs){
        ...
        if($request->hasFile('offer_photo')){
            /*for($x=0; $x < count($request->file('offer_photo')); $x++){*/
                $arr['offer_photo.*'] = 'image|dimensions:min_width=1200,min_height=700,max_width=1300,max_height=800';
            /*}*/
        }
    ...
    protected function getCreateMessages(){
        return [
            ...
            'city_id.*' => __('backend.Please select City'),
